...
def build_model(cfg: dict = None,
                ...
                trg_vocab: Vocabulary = None) -> Tuple[Encoder, Decoder]:
    """
    Build and initialize the encoder and decoder according to the configuration.

    :param cfg: dictionary configuration containing model specifications
    :param src_vocab: source vocabulary
    :param trg_vocab: target vocabulary
    :return: built and initialized encoder and decoder
    """
    ...
    if cfg.get("tied_softmax", False):
        if trg_embed.lut.weight.shape == decoder.output_layer.weight.shape:
            # (also) share trg embeddings and softmax layer:
            decoder.output_layer.weight = trg_embed.lut.weight
        else:
            raise ConfigurationError(
    ...
    initialize_model(Model(encoder, decoder, src_embed, trg_embed, src_vocab, trg_vocab), cfg, src_pad_index, trg_pad_index)
    ...
    if enc_embed_path:
        ...
        src_embed.load_from_file(Path(enc_embed_path), src_vocab)
    if dec_embed_path and not cfg.get("tied_embeddings", False):
        logger.info("Loading pretrained trg embeddings...")
        trg_embed.load_from_file(Path(dec_embed_path), trg_vocab)
